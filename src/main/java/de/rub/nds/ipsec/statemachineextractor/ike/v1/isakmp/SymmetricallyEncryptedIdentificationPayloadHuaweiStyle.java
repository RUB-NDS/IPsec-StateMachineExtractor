...
public class SymmetricallyEncryptedIdentificationPayloadHuaweiStyle extends SymmetricallyEncryptedISAKMPPayload {
    ...
    public static SymmetricallyEncryptedIdentificationPayloadHuaweiStyle fromStream(ByteArrayInputStream bais, IKEv1Ciphersuite ciphersuite, SecretKeySpec ke, byte[] iv) throws GenericIKEParsingException {
        try {
            ...
            encPayload.encryptedBody = Arrays.copyOfRange(buffer, IdentificationPayload.ID_HEADER_LEN - GENERIC_PAYLOAD_HEADER_LEN, buffer.length);
            encPayload.decrypt();
            return encPayload;
    ...
    public int getLength() {
        return super.getLength() + IdentificationPayload.ID_HEADER_LEN - GENERIC_PAYLOAD_HEADER_LEN; // That's a +4 with context
    }
